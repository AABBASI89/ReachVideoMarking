%% VIDEO ANALYSIS GUI
% Shakthi Visagan
% This script and GUI was designed for the larger laptops.

clc; clear; close all;

%% FIGURE WINDOW

% need to clear pre-existing objects (if any)
clear screen_info screen_width screen_height white_space_prop;
clear vid_fig_hand
clear vid_fig_pos;

global vid_fig_hand;

% left	 Distance from the left edge of the primary display to the inner left edge of the window. This value can be negative on systems that have more than one monitor.
% bottom Distance from the bottom edge of the primary display to the inner bottom edge of the window. This value can be negative on systems that have more than one monitor.
% width	 Distance between the right and left inner edges of the figure.
% height Distance between the top and bottom inner edges of the window.

left = 100;
bottom = 100;
width =  1550;
height = 800;
clear screen_width screen_height white_space_prop;

vid_fig_pos=[left bottom width height];
clear left bottom width height;

vid_fig_hand.fig = figure('Position',vid_fig_pos,...
    'MenuBar','none',...
    'NumberTitle','off',...
    'CloseRequestFcn',@vid_fig_close_Callback,...
    'Name','Reach-to-Grasp Task Video Analysis GUI');

set(vid_fig_hand.fig, 'WindowKeyPressFcn', @vid_fig_hand_fig_WindowKeyPressFcn);

%% Notes about GUI

% for Positions:
% [left bottom width height]
% left	 Distance from the inner left edge of the parent container to the outer left edge of the user interface control
% bottom Distance from the inner bottom edge of the parent container to the outer bottom edge of the user interface control
% width	 Distance between the right and left outer edges of the user interface control
% height Distance between the top and bottom outer edges of the user interface control

%% GET VID_DIR and VID_DIR

clear VID_DIR;


VID_DIR = pwd;
addpath(VID_DIR); % add to the path the directory of the videos

vid_fig_hand.VID_DIR_txt_handle = uicontrol(vid_fig_hand.fig,...
    'Style','text',...
    'Position',[10 750 190 40],...
    'String','Video Directory:');

set(vid_fig_hand.VID_DIR_txt_handle,'FontSize',20);
set(vid_fig_hand.VID_DIR_txt_handle, 'TooltipString', 'Enter the name of the directory in which the videos are saved.')


vid_fig_hand.VID_DIR_handle = uicontrol(vid_fig_hand.fig,...
    'Style','edit',...
    'Position',[200 755 850 30]);

set(vid_fig_hand.VID_DIR_handle,'String',VID_DIR);
set(vid_fig_hand.VID_DIR_handle,'FontSize',12);
set(vid_fig_hand.VID_DIR_handle,'BackgroundColor',[0.4 0.7 0.6 ]);
set(vid_fig_hand.VID_DIR_handle, 'TooltipString', 'Enter the name of the directory in which the videos are saved.')

clear VID_DIR;
clear VID_DIR_cell;

%% RAT ID

vid_fig_hand.NAME_txt_handle = uicontrol(vid_fig_hand.fig,...
    'Style','text',...
    'Position',[10 715 175 40],...
    'String','Subject Name:');

set(vid_fig_hand.NAME_txt_handle,'FontSize',20);
set(vid_fig_hand.NAME_txt_handle, 'TooltipString', 'Enter the name of the rat of interest.')

vid_fig_hand.NAME_handle = uicontrol(vid_fig_hand.fig,...
    'Style','edit',...
    'Position',[185 720 175 30]);

set(vid_fig_hand.NAME_handle,'String','Txxx');
set(vid_fig_hand.NAME_handle,'FontSize',20);
set(vid_fig_hand.NAME_handle,'BackgroundColor',[0 0.7 1 ]);
set(vid_fig_hand.NAME_handle,'String','Txxx');
set(vid_fig_hand.NAME_handle, 'TooltipString', 'Enter the name of the rat here. This is usually in the form of Txxx. For example T003 is a a valid example, if that rat exists.')

%% Date

clear todayDateString;

vid_fig_hand.dateTextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Date:', ...
    'TooltipString', 'Enter the date of the trials which you would like to analyze. For example, 03-Apr-2019 is a valid date, if trials were conducted on that day.',...
    'Horizontalalignment', 'left', ...
    'Position', [10 680 66 40]);

vid_fig_hand.dateEditBoxHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Edit', ...
    'Position', [76 685 175 30], ...
    'TooltipString', 'Date in the format of two number day, three letter month, four number year. For example, 14-Mar-2015 and 25-Nov-2019 are both valid.',...
    'BackgroundColor', [0.8 0.2 0.5 ]);

todayDateString = date;
set(vid_fig_hand.dateEditBoxHandle,'String',todayDateString);
set(vid_fig_hand.dateEditBoxHandle,'FontSize',20);
clear todayDateString;

%%

vid_fig_hand.timeTextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Time:', ...
    'TooltipString', 'Enter the time of the trials which you would like to analyze. For example, 16h39m is a valid time, if trials were conducted during that time.',...
    'Horizontalalignment', 'left', ...
    'Position', [10 645 76 40]);

vid_fig_hand.timeEditBoxHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Edit', ...
    'Position', [86 650 175 30], ...
    'TooltipString', 'Date in the format of two number hour, two number minute. For example, 16h39m is a valid time, if trials were conducted during that time.',...
    'BackgroundColor', [0.5 0.8 0.5 ]);

set(vid_fig_hand.timeEditBoxHandle,'String', '00h00m');
set(vid_fig_hand.timeEditBoxHandle,'FontSize',20);


%%

vid_fig_hand.listFiles_handle = uicontrol(vid_fig_hand.fig,...
    'Style','listbox',...
    'Position',[10 340 260 200],...
    'String',{'Welcome...'},...
    'FontSize',9);

%%

vid_fig_hand.findFiles = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','FIND FILES',...
    'Position',[10 545 66 30],...
    'Callback',@findFiles_Callback);


%%

vid_fig_hand.previewVid = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','PREVIEW',...
    'Position',[10 305 66 30],...
    'Callback',@previewVid_Callback);

%%

% ax = axes('Position', [left bottom width height])
% Note that the values are specified in normalized units, 
% i.e. the position values can change from 0 to 1.

vid_fig_hand.vid_ax=axes('Position',[0.1825 0.18 1.1/2.75 (1.2*640/480/2.75)]);
set(vid_fig_hand.vid_ax, 'XTick', []);
set(vid_fig_hand.vid_ax, 'YTick', []);
set(vid_fig_hand.vid_ax, 'Xcolor', 'k');
set(vid_fig_hand.vid_ax, 'Ycolor', 'k');
set(vid_fig_hand.vid_ax, 'Color', 'k');
title(vid_fig_hand.vid_ax, 'Frame Control');
%set(vid_fig_hand.vid_ax, 'ButtonDownFcn', @vid_fig_hand_vid_ax_ButtonDownFcn);


%%

vid_fig_hand.placeVid = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','PLACE',...
    'Position',[79 305 66 30],...
    'Callback',@placeVid_Callback);

%%

vid_fig_hand.markFrameReach = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Mark Reach Frame',...
    'Position',[160 200 106 30],...
    'Callback',@markFrameReach_Callback);

%%

vid_fig_hand.markFullReach = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Mark Full Trajectory',...
    'Position',[160 233 106 30],...
    'Callback',@markFullReach_Callback);

%%

vid_fig_hand.frameEdithandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'Position', [511 85 150 30], ...
    'BackgroundColor', [0.5 0.9 0.7 ]);

set(vid_fig_hand.frameEdithandle,'String',1);
set(vid_fig_hand.frameEdithandle,'FontSize',20);

%%

vid_fig_hand.plusOneFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','(+1 Frame) >',...
    'Position',[665 85 66 30],...
    'Callback',@plusOneFrame_Callback);

set(vid_fig_hand.plusOneFrame, 'TooltipString', 'Enter just <right arrow> to move up 1 (one) frame.')

%%

vid_fig_hand.minusOneFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','< (-1 Frame)',...
    'Position',[441 85 66 30],...
    'Callback',@minusOneFrame_Callback);

set(vid_fig_hand.minusOneFrame, 'TooltipString', 'Enter just <left arrow> to move down 1 (one) frame.');

%%
vid_fig_hand.plusTenFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','(+10 Frame) >>',...
    'Position',[734 85 78 30],...
    'Callback',@plusTenFrame_Callback);

set(vid_fig_hand.plusTenFrame, 'TooltipString', 'Enter <Shift> and <right arrow> to move up 10 (ten) frames.');

%%

vid_fig_hand.minusTenFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','<< (-10 Frame)',...
    'Position',[360 85 78 30],...
    'Callback',@minusTenFrame_Callback);

set(vid_fig_hand.minusTenFrame, 'TooltipString', 'Enter <Shift> and <left arrow> to move down 10 (ten) frames.');

%%

vid_fig_hand.plusHunderedFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','(+100 Frame) >>',...
    'Position',[815 85 78 30],...
    'Callback',@plusHunderedFrame_Callback);

set(vid_fig_hand.plusHunderedFrame, 'TooltipString', 'Enter <Alt> and <right arrow> to move up 100 (ten) frames.')

%%
vid_fig_hand.minusHunderedFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','<< (-100 Frame)',...
    'Position',[278 85 78 30],...
    'Callback',@minusHunderedFrame_Callback);

set(vid_fig_hand.minusHunderedFrame, 'TooltipString', 'Enter <Alt> and <left arrow> to move down 100 (ten) frames.')
%%

vid_fig_hand.lastFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','(Last Frame) >|',...
    'Position',[895 85 78 30],...
    'Callback',@lastFrame_Callback);

set(vid_fig_hand.lastFrame, 'TooltipString', 'Click to go to the last frame.')

%%

vid_fig_hand.firstFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','|< (First Frame)',...
    'Position',[195 85 78 30],...
    'Callback',@firstFrame_Callback);

set(vid_fig_hand.firstFrame, 'TooltipString', 'Click to go to the first frame.')
%%

vid_fig_hand.goToFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Go to Frame #',...
    'Position',[545 52 78 30],...
    'Callback',@goToFrame_Callback);

set(vid_fig_hand.goToFrame, 'TooltipString', 'Click to go the frame written in the box above.')

%%

vid_fig_hand.frameEditGoTohandle = uicontrol(vid_fig_hand.fig, 'Style', 'Edit', ...
    'Position', [636 52 78 30], ...
    'BackgroundColor', [0.2 0.9 0.5 ]);

set(vid_fig_hand.frameEditGoTohandle,'String',1);
set(vid_fig_hand.frameEditGoTohandle,'FontSize',20);

%%

vid_fig_hand.uit = uitable(vid_fig_hand.fig,...
    'Position',[906 370 410 204]);
vid_fig_hand.uit.ColumnName = {'Trial' 'Frame of Reach Onset', 'Trial Outcome', 'Reach Trajectory'};
vid_fig_hand.uit.ColumnEditable = false;


%%

vid_fig_hand.logEventFrame = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Log Events',...
    'Position',[1322 315 110 30],...
    'Callback',@logEventFrame_Callback);

%%

vid_fig_hand.RowEditHd = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'String','1',...
    'FontSize',20,...
    'Position', [1125 315 85 30], ...
    'BackgroundColor', [0.8 0.5 0.86 ]);

%%

vid_fig_hand.saveTable = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Save Table',...
    'Position',[1322 515 110 30],...
    'Callback',@saveTable_Callback);

%%

vid_fig_hand.getResults = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','Get Results',...
    'Position',[1322 415 110 30],...
    'Callback',@getResults_Callback);

%%

vid_fig_hand.RowEditHdRCHONSET = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'Position', [1125 223 340 30], ...
    'BackgroundColor', [0.7 0.99 0.6 ]);

set(vid_fig_hand.RowEditHdRCHONSET,'String','');
set(vid_fig_hand.RowEditHdRCHONSET,'FontSize',20);

%%

vid_fig_hand.RowEditHdTRLOUTCOME = uicontrol(vid_fig_hand.fig, 'Style', 'Edit', ...
    'Position', [1125 280 85 30], ...
    'BackgroundColor', [0.6 0.99 1 ]);

set(vid_fig_hand.RowEditHdTRLOUTCOME,'String','2');
set(vid_fig_hand.RowEditHdTRLOUTCOME,'FontSize',20);

%% Push button to log unsuccessful trials
vid_fig_hand.logUnsuccTrials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','U : 0',...
    'Position',[1210 280 50 30],...
    'Callback',@logUnsuccTrials_Callback);

%% Push button to log successful trials
vid_fig_hand.logSuccTrials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','S : 1',...
    'Position',[1260 280 50 30],...
    'Callback',@logSuccTrials_Callback);

%% Push button to log ignored trials
vid_fig_hand.logIgTrials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','I : 2',...
    'Position',[1310 280 50 30],...
    'Callback',@logIgTrials_Callback);

%% Push button to log reach wo pellet trials
vid_fig_hand.logOut3Trials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','O : 3',...
    'Position',[1360 280 50 30],...
    'Callback',@logOut3Trials_Callback);

%% Push button to log reach on forcep unsuccessful trials
vid_fig_hand.logOut4Trials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','O : 4',...
    'Position',[1410 280 50 30],...
    'Callback',@logOut4Trials_Callback);

%% Push button to log reach on forcep successful trials
vid_fig_hand.logOut5Trials = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String','O : 5',...
    'Position',[1460 280 50 30],...
    'Callback',@logOut5Trials_Callback);

%%

vid_fig_hand.TRIALOT_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Trial Outcome: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [915 280 185 30]);

%%

vid_fig_hand.RowEditHdPLTTOUCH = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'Position', [1125 188 85 30], ...
    'BackgroundColor', [0.6 0.6 1 ]);

set(vid_fig_hand.RowEditHdPLTTOUCH,'String','');
set(vid_fig_hand.RowEditHdPLTTOUCH,'FontSize',20);

%%

vid_fig_hand.RowEditHdRTRCTONSET = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'Position', [1125 153 85 30], ...
    'BackgroundColor', [0.7 0.7 0.7 ]);

set(vid_fig_hand.RowEditHdRTRCTONSET,'String','');
set(vid_fig_hand.RowEditHdRTRCTONSET,'FontSize',20);

%%

vid_fig_hand.TRIALNO_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Trial #: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [1010 315 85 30]);

%%

vid_fig_hand.RCHFRAMENO_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Reach Onset: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [927 223 185 30]);

%%

vid_fig_hand.TCHFRAMENO_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Pellet Touch: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [934 188 185 30]);

%%

vid_fig_hand.RTRCTFRAMENO_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 20,...
    'String', 'Retract Onset: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [918 153 185 30]);

%%

vid_fig_hand.TRAJCHECKBOX = uicontrol(vid_fig_hand.fig, 'Style', 'checkbox', ...
   'Position',[252 167 14 30],...
   'Value', true);

vid_fig_hand.TRAJCHECKBOX_TextHandle = uicontrol(vid_fig_hand.fig, 'Style', 'Text', ...
    'FontSize', 10,...
    'String', 'Trajectory Required: ', ...
    'Horizontalalignment', 'left', ...
    'Position', [132 160 120 30]);

%%

vid_fig_hand.ErrDispText = uicontrol(vid_fig_hand.fig,...
    'Style','text',...
    'String', '', ...
    'FontSize', 10,...
    'Horizontalalignment', 'center', ...
    'Position', [918 80 520 65],...
    'Visible', false);

%'Verticalalignment', 'center', ...
    
vid_fig_hand.ConfOverwrite = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String', 'Overwrite', ...
    'Horizontalalignment', 'center', ...
    'Position', [1003 45 85 30],...
    'Visible', false,...
    'Callback',@ConfOverwrite_Callback);

vid_fig_hand.CancelLog = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String', 'Cancel', ...
    'Horizontalalignment', 'center', ...
    'Position', [1273 45 85 30],...
    'Visible', false,...
    'Callback',@CancelLog_Callback);

vid_fig_hand.ChangeTrialNum = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String', 'Set new trial #:', ...
    'Horizontalalignment', 'center', ...
    'Position', [1093 45 85 30],...
    'Visible', false,...
    'Callback',@ChangeTrialNum_Callback);

vid_fig_hand.AltTrialNum = uicontrol(vid_fig_hand.fig,...
    'Style','edit',...
    'FontSize', 20,...
    'Horizontalalignment', 'center', ...
    'BackgroundColor', [.95 0.5 1 ],...
    'Position', [1183 45 85 30],...
    'Visible', false);

vid_fig_hand.ErrDispClear = uicontrol(vid_fig_hand.fig,...
    'Style','pushbutton',...
    'String', 'Clear', ...
    'Horizontalalignment', 'center', ...
    'Position', [1138 45 85 30],...
    'Visible', false,...
    'Callback',@ErrDispClear_Callback);

%% EOF